...
def centroide(contours):
    ...
    if M["m00"] > 1e-3:  # Evitar división por cero
        ...
        # Y crece hacia arriba desde el origen
        y_m = (origen_y - cy) / px_per_m
        ...
        cv.putText(frame, "O(0,0)", (int(origen_x)+10, int(origen_y)-10),
                   cv.FONT_HERSHEY_SIMPLEX, 0.5, (0, 255, 0), 2)

        # Mostrar coordenadas relativas al origen
        cv.putText(frame, f"({x_m:.2f}, {y_m:.2f})m", (int(
            cx)+10, int(cy)), cv.FONT_HERSHEY_SIMPLEX, 0.4, (255, 255, 255), 1)

        print(
            f"Frame {frame_num}: Pelota en ({cx:.1f}, {cy:.1f}) px = ({x_m:.3f}, {y_m:.3f}) m desde origen")
...
def calibrar_origen(first_frame):
    ...
    def mouse_callback_origen(event, x, y, flags, param):
        if event == cv.EVENT_LBUTTONDOWN:
            ...
            cv.putText(img_temp, "ORIGEN (0,0)", (x+10, y-10),
...
def dibujar_trayectoria(frame, positions, color=(0, 255, 255), grosor=2):
    ...
    for i in range(1, len(positions)):
        if positions[i-1][1] is not None and positions[i][1] is not None:
            # (cx_anterior, cy_anterior)
            pt1 = (int(positions[i-1][1]), int(positions[i-1][2]))
            # (cx_actual, cy_actual)
            pt2 = (int(positions[i][1]), int(positions[i][2]))
...
def mostrar_informacion_fisica(frame, positions, vels, accs, t):
    ...
    overlay = frame.copy()
    cv.rectangle(overlay, (frame.shape[1] - 200, 10),
    ...
    if len(positions) > 5:  # Después de algunos frames
        ...
        if vels:
            ...
            v0_estimada = math.sqrt(
                vels[0][1]**2 + vels[0][2]**2) if vels else 0
            angulo_estimado = math.degrees(math.atan2(
                vels[0][2], vels[0][1])) if vels else 0
...
def calcular_datos_teoricos():
    ...
    print(f"\n🔍 VERIFICACIÓN:")
    print(
        f"   • Posición final calculada: ({datos['x_m'].iloc[-1]:.3f}, {datos['y_m'].iloc[-1]:.3f}) m")
...
def generar_graficas_comparativas():
    ...
    if vels:  # Si hay datos de velocidad experimental
        ...
        vx_exp = [v[1] for v in vels]
        vy_exp = [v[2] for v in vels]
        v_total_exp = [math.sqrt(vx**2 + vy**2)
                       for vx, vy in zip(vx_exp, vy_exp)]
    ...
    plt.tight_layout()
    plt.savefig('comparacion_teorico_experimental.png',
    ...
    if t_exp.max() <= t_teo.max() and t_exp.min() >= t_teo.min():
        ...
        print(
            f"🎯 Error promedio en X: {np.mean(np.abs(error_x)):.4f} ± {np.std(error_x):.4f} m")
        print(
            f"🎯 Error promedio en Y: {np.mean(np.abs(error_y)):.4f} ± {np.std(error_y):.4f} m")
        print(
            f"🎯 Error total promedio: {np.mean(error_total):.4f} ± {np.std(error_total):.4f} m")
...
if cap is not None:
    ...
    if ok:
        ...
        out = cv.VideoWriter('video_salida.mp4', fourcc, fps, (320, 480))
        ...
        while frame_num < 21:
            ...
            dibujar_trayectoria(
                frame, positions, color=(0, 255, 255), grosor=2)
            ...
            mostrar_informacion_fisica(frame, positions, vels, accs, t)
        ...
        print(f"💾 Video guardado en: video_salida.mp4")

        # Guardar en archivo CSV
        if positions:
            df_positions = pd.DataFrame(
                positions, columns=['tiempo_s', 'cx_px', 'cy_px', 'x_m', 'y_m'])
        ...
        if vels:
            ...
            if len(vels) > 1:
                try:
                    ...
                    # Velocidad total
                    df_positions['v_total_ms'] = np.sqrt(
                        df_positions['vx_ms']**2 + df_positions['vy_ms']**2)
